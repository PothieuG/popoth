...
export const POST = withAuthAndProfile(async (request, { profile }) => {
  try {
    ...
    console.log(
      `${action === 'deduct_from_budget' ? `🎯 BUDGET UTILISÉ: ${budget_id}` : '🎯 MODE: Report sur le mois suivant'}`,
    )
    console.log(``)
    ...
    if (action === 'carry_forward') {
      ...
      if (!budget) {
        return NextResponse.json({ error: 'Budget spécifié non trouvé' }, { status: 404 })
      }
    ...
    try {
      ...
      if (existingRecap) {
        ...
        console.log(
          `🔄 [Monthly Recap Complete] Mise à jour du récap existant ID: ${existingRecap.id}`,
        )
      ...
      console.log(
        `📝 [Monthly Recap Complete] Le solde bancaire reste inchangé (le reste à vivre est déjà pris en compte dans les calculs)`,
      )

      // 2.1. NE PAS remettre les revenus estimés à 0 - ils restent pour le mois suivant
      console.log(
        `📝 [Monthly Recap Complete] Les revenus estimés restent inchangés pour le mois suivant`,
      )
      ...
      console.log(
        `🔄 [Deficit Processing] Début du traitement des déficits pour ${context}:${contextId}`,
      )
      ...
      try {
        ...
        if (budgetsQueryError) {
          console.error(
            '❌ [Deficit Processing] Erreur lors de la récupération des budgets:',
            budgetsQueryError,
          )
          ...
          console.log(
            `🔄 [Deficit Processing] ${allBudgets.length} budget(s) trouvé(s), calcul des déficits...`,
          )
          ...
          if (transfersError) {
            console.error(
              '❌ [Deficit Processing] Erreur lors de la récupération des transferts:',
              transfersError,
            )
          }
          ...
          for (const budget of allBudgets) {
            ...
            console.log(
              `📊 [Deficit Processing] "${budget.name}": ${budget.estimated_amount}€ estimé, ${realExpensesThisMonth}€ dépensé, transferts (from: ${transfersFrom}€, to: ${transfersTo}€), ajusté: ${adjustedSpentAmount}€ = ${deficit}€ déficit`,
            )
          ...
          global.postTransferBudgetDeficit = global.carryoverUpdates.reduce(
            (sum: number, u) => sum + u.carryover_amount,
            0,
          )
          console.log(
            `📊 [Deficit Processing] Déficit pré-transfert: ${preTransferBudgetDeficit}€, post-transfert: ${global.postTransferBudgetDeficit}€`,
          )

          console.log(
            `🔄 [Deficit Processing] ${global.carryoverUpdates.length} budget(s) à mettre à jour après reset`,
          )
        }
      } catch (deficitError) {
        console.error(
          '❌ [Deficit Processing] Erreur générale lors du traitement des déficits:',
          deficitError,
        )
      ...
      console.log(
        `ℹ️ [Savings Processing] Les surplus seront transférés vers économies dans la section 3.7`,
      )

      // 3.6. Calculer l'écart de reste à vivre et créer une dépense exceptionnelle si nécessaire
      console.log(
        `📊 [RAV Difference Processing] Début du calcul de l'écart de reste à vivre pour ${context}:${contextId}`,
      )

      try {
        // Calculer le reste à vivre de base: revenus estimés - dépenses estimées
        const baseRemainingToLive =
        ...
        if (bankBalanceError) {
          console.error(
            '❌ [RAV Difference Processing] Erreur lors de la récupération du reste à vivre actuel:',
            bankBalanceError,
          )
        } else {
          ...
          console.log(
            `📊 [RAV Difference Processing] Reste à vivre de base (calcul): ${baseRemainingToLive}€`,
          )
          console.log(
            `📊 [RAV Difference Processing] Reste à vivre actuel (BDD): ${currentRemainingToLive}€`,
          )
          ...
          const deficitCoveredByTransfers =
            (global.preTransferBudgetDeficit || 0) - (global.postTransferBudgetDeficit || 0)
          const adjustedDifference = difference + deficitCoveredByTransfers

          console.log(
            `📊 [RAV Difference Processing] Déficit couvert par transferts: ${deficitCoveredByTransfers}€`,
          )
          ...
          if (adjustedDifference < 0) {
            ...
            console.log(
              `📝 [RAV Difference Processing] Création d'une dépense exceptionnelle de ${exceptionalExpenseAmount}€ pour le mois prochain`,
            )
            ...
            console.log(
              `✅ [RAV Difference Processing] Dépense exceptionnelle préparée pour insertion`,
            )
          } else {
            console.log(
              `✅ [RAV Difference Processing] Pas d'écart négatif, aucune dépense exceptionnelle à créer`,
            )
          }
        }
      } catch (ravDifferenceError) {
        console.error(
          "❌ [RAV Difference Processing] Erreur générale lors du traitement de l'écart de reste à vivre:",
          ravDifferenceError,
        )
        // Ne pas faire échouer la transaction pour ça
      }
      ...
      console.log(
        `💰 [Savings Processing] Calcul et transfert des surplus vers économies pour ${context}:${contextId}`,
      )
      ...
      try {
        ...
        if (budgetsForSavingsError) {
          console.error(
            '❌ [Savings Processing] Erreur récupération budgets:',
            budgetsForSavingsError,
          )
          ...
          for (const budget of budgetsForSavings) {
            ...
            if (surplus > 0) {
              ...
              if (updateSavingsError) {
                console.error(
                  `❌ [Savings Processing] Erreur mise à jour économies "${budget.name}":`,
                  updateSavingsError,
                )
              } else {
                console.log(
                  `✅ [Savings Processing] "${budget.name}": surplus ${surplus}€ → économies (${budget.cumulated_savings || 0}€ → ${newCumulatedSavings}€)`,
                )
                totalSurplusTransferred += surplus
              }
            } else {
              console.log(
                `ℹ️ [Savings Processing] "${budget.name}": aucun surplus à transférer (dépensé: ${adjustedSpent}€ / estimé: ${budget.estimated_amount}€)`,
              )
            }
          }

          console.log(
            `💰 [Savings Processing] Total surplus transféré vers économies: ${totalSurplusTransferred}€`,
          )
        }
      } catch (savingsError) {
      ...
      console.log(
        `🗑️ [Monthly Recap Complete] Suppression des revenus réels pour ${context}:${contextId}`,
      )
      ...
      console.log(
        `🗑️ [Monthly Recap Complete] Suppression des dépenses réelles pour ${context}:${contextId}`,
      )
      ...
      if (deleteRealExpensesError) {
        console.error(
          '❌ Erreur lors de la suppression des dépenses réelles:',
          deleteRealExpensesError,
        )
      } else {
      ...
      console.log(
        `🗑️ [Monthly Recap Complete] Suppression des transferts entre budgets pour ${context}:${contextId}`,
      )
      ...
      if (global.carryoverUpdates && global.carryoverUpdates.length > 0) {
        console.log(
          `🔄 [Deficit Processing] Application du carryover sur ${global.carryoverUpdates.length} budget(s)`,
        )
        ...
        for (const update of global.carryoverUpdates) {
          ...
          if (updateCarryoverError) {
            console.error(
              `❌ [Deficit Processing] Erreur carryover "${update.budget_name}":`,
              updateCarryoverError,
            )
          } else {
            console.log(
              `✅ [Deficit Processing] "${update.budget_name}": carryover_spent_amount = ${update.carryover_amount}€`,
            )
          }
        }
      ...
      if (global.exceptionalExpenseToInsert) {
        ...
        if (insertExceptionalError) {
          console.error(
            "❌ [RAV Difference Processing] Erreur lors de l'insertion de la dépense exceptionnelle:",
            insertExceptionalError,
          )
        } else {
          console.log(
            `✅ [RAV Difference Processing] Dépense exceptionnelle de ${global.exceptionalExpenseToInsert.amount}€ insérée avec succès`,
          )
        }
      ...
      const summary = {
        ...
        budget_used:
          action === 'deduct_from_budget'
            ? currentBudgets?.find((b) => b.id === budget_id)?.name
            : null,
        total_surplus: totalSurplus,
        total_deficit: totalDeficit,
        incomes_reset: true,
