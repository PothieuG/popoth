...
L'audit complet est dans [`docs/audit/00-executive-summary.md`](./docs/audit/00-executive-summary.md). État après **Sprint Cleanup-I8 / Lot 4a (logger migration `app/api/groups/**` avec philosophie de triage)** 2026-05-10 (**~98.2/100**) :
...
- [`prompt/`](./prompt/) — prompts Claude Code par sprint, du Sprint 0 (v0) à Sprint Cleanup-I8 / Lot 4a (livré 2026-05-10). Voir [`prompt/README.md`](./prompt/README.md) pour le sommaire chronologique. **Prochain prompt pré-rédigé** : [`prompt-07-deep-dive-console-log-cleanup-v4.md`](./prompt/prompt-07-deep-dive-console-log-cleanup-v4.md) — Lot 4b (migration `app/api/monthly-recap/**` routes simples, 9 fichiers, ~203 sites, ratio attendu ~85% DROP / ~15% KEEP+migrate). La roadmap longue planifiée dans [CLAUDE.md](./CLAUDE.md) §11 reste la référence canonique (Sprint Tailwind-v4, Sprint Supabase-Strict-Types, chantier I4 `financial-calculations`, chantier I5 `process-step1`, Lots 4b-6 du chantier console.log cleanup, chantier Zod rollout, GH Actions Node.js 24 migration juin 2026).
