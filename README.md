...
L'audit complet est dans [`docs/audit/00-executive-summary.md`](./docs/audit/00-executive-summary.md). État après **Sprint 2-followup-v5 (`useAuth.ts` dead code purge)** 2026-05-10 (**~98.2/100**) :
...
- [`prompt/`](./prompt/) — prompts Claude Code par sprint, du Sprint 0 (v0) à Sprint Cleanup-I8 / Lot 1 (livré 2026-05-10). Voir [`prompt/README.md`](./prompt/README.md) pour le sommaire chronologique. **Prochain prompt pré-rédigé** : [`prompt-07-deep-dive-console-log-cleanup-v2.md`](./prompt/prompt-07-deep-dive-console-log-cleanup-v2.md) — tests pure-unit pour `lib/logger.ts` + Lot 3 (migration `middleware.ts` + `lib/expense-allocation.ts`). La roadmap longue planifiée dans [CLAUDE.md](./CLAUDE.md) §11 reste la référence canonique (Sprint Tailwind-v4, Sprint Supabase-Strict-Types, chantier I4 `financial-calculations`, chantier I5 `process-step1`, Lots 2/4-6 du chantier console.log cleanup, chantier Zod rollout, GH Actions Node.js 24 migration juin 2026).
