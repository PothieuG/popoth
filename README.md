...
lib/
  ...
  api/
    with-auth.ts           # ✅ Sprint Refactor-Architecture-v3 — withAuth + withAuthAndProfile higher-order helpers (auth + profile fetch en 1 ligne par handler)
...
- **Auth boilerplate extrait** depuis Sprint Refactor-Architecture-v3 : tous les handlers `lib/api/finance/*` utilisent `withAuth(handler)` (auth seule, passe `{ userId }`) ou `withAuthAndProfile(handler)` (auth + fetch `select('id, group_id')`, passe `{ userId, profile }`) depuis [`lib/api/with-auth.ts`](./lib/api/with-auth.ts). Le wrapper rejette les requêtes non authentifiées avec `401 'Session invalide'` (CLAUDE.md §6). **Pas de try/catch dans le wrapper** — chaque handler garde son `try/catch` route-aware avec `console.error` ; permet aussi à `summary.ts` de préserver son fallback 200-with-default-data en cas d'erreur. Pour un nouveau handler finance, suivre le pattern : `export const POST = withAuth(async (request, { userId }) => { ... })` ou `export const POST = withAuthAndProfile(async (request, { userId, profile }) => { ... })`.
...
L'audit complet est dans [`docs/audit/00-executive-summary.md`](./docs/audit/00-executive-summary.md). État après Sprint Refactor-Architecture-v3 2026-05-08 (~91/100) :
...
- [`prompts/`](./prompts/) — prompts Claude Code par sprint, du Sprint 0 (v0) à Sprint Refactor-Architecture-v3 (livré 2026-05-08). Voir [`prompts/README.md`](./prompts/README.md) pour le sommaire chronologique. Voir roadmap §11 dans [CLAUDE.md](./CLAUDE.md) pour les chantiers planifiés : **Sprint Refactor-Architecture-v4** (extension du wrapper `withAuth` à `/api/{profile,groups,savings,monthly-recap}/**` hors god route — voir [`prompts/prompt-03-architecture-v4.md`](./prompts/prompt-03-architecture-v4.md)), Sprint 1 (Prettier/Husky lint-staged + eslint-config-next 15→16), Sprint Tailwind-v4, Sprint Supabase-Strict-Types, chantier I4 (financial-calculations), chantier I5 (process-step1), chantier console.log cleanup, chantier Zod rollout, GH Actions Node.js 24 migration (juin 2026).
