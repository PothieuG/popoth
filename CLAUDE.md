...
| Tests non-gated passants               | **599**                   | `pnpm test:run`                                                                                                          |
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 599 non-gated / 164 gated. 38 routes API. 16 RPCs pinnées + 23 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` (rappel "relire 00-README + 00-Detailed_feature" en tête de chaque prompt 01-17), sprints 01-14/17 livrés (cf. §5 + Part 22).

**Historique** — 22 parts `.claude/history/roadmap-detailed-NN-*.md` (126 sprints) :
