...
- **Monthly Recap V3** : en cours d'implémentation, voir `prompt-montly-recap/` pour la spec. Sprints 01 Clean-Slate-Recap (2026-05-23) + 02 Migrations-V3 (2026-05-24, table `monthly_recaps` + state machine + lock + snapshot JSONB + carry-over flags) + 03 State+Schemas (2026-05-24) + 04 Calculs purs (2026-05-23, commit ec47594) + 05 Endpoints START+STATUS+proxy (2026-05-25) livrés. Fondations TS dans [lib/recap/](lib/recap/) : `state.ts` (RecapStep + isAdvanceAllowed + nextRequiredStep), `check-status.ts` (discriminated union `RecapStatusKind` + `.maybeSingle()` + lock detection via `started_by_profile_id`), `lock.ts` (helpers purs `isUserLocked`/`isRecapBlocking`), `calculations.ts` (4 fonctions pures cents-precise : `computeBudgetSurplus`, `computeRecapSummary`, `computeProportionalSavingsRefloat`, `computeProportionalBudgetSnapshot` Option B pool=estimatedAmount), `load-summary.ts` (Promise.all FinancialData + budgets + spent-by-budget calendar-filtered + piggy + bank `.maybeSingle()` → `computeRecapSummary`), `types.ts` (interfaces partagées). RPC atomique `start_monthly_recap` (`INSERT ON CONFLICT DO NOTHING` + discriminant `result` ∈ `created|resumed|completed|locked_by_other`). 2 endpoints `app/api/monthly-recap/{start,status}/route.ts` via `withAuthAndProfile` + `parseBody`/`parseQuery`. **Gating proxy.ts actif** : `/dashboard` et `/group-dashboard` redirigent sur `/monthly-recap?context=X` tant que `isRecapBlocking(status)` ; cookie httpOnly 5min `recap-ok-{ctx}-{YYYY}-{MM}` cache l'état `completed` pour éviter le hit DB par nav. Schémas Zod des 8 endpoints futurs dans [lib/schemas/recap.ts](lib/schemas/recap.ts).
...
| Tests gated skipped (sans env vars)    | **101**                   | idem (sprint 05 V3 : +13 gated start (7) + status (6) sous `SUPABASE_RECAP_TESTS=1`)                                     |
| Routes API                             | **31**                    | `pnpm build` (sprint 05 V3 : +POST /api/monthly-recap/start, +GET /api/monthly-recap/status)                             |
...
- **SUPABASE_RECAP_TESTS=1** : 21 cas V3 — 8 `checkRecapStatus` (4 fixtures + orphan + 2 errors) + sprint 05 routes : 7 `POST /api/monthly-recap/start` (created/resumed/orphan/group-created/locked_by_other/completed/no-group) + 6 `GET /api/monthly-recap/status` (no_recap/in_progress+summary/completed/locked_by_other/no-group/missing-query).

### Tests non-gated par module

Couverture par dossier : `lib/finance/` (calc-rtl 19 + snapshots 5), `lib/api/` (parse-body 9, finance/expenses-add-with-logic 5 PIN ATOMIC CONTRACT), `app/api/savings/transfer/` (4 PIN ATOMIC CONTRACT), `app/api/monthly-recap/` (sprint 05 V3 : 7 gated start + 6 gated status, `withAuthAndProfile` mocké), `lib/schemas/` (10 fichiers : 9 post Clean-Slate + recap V3 40 cas), `lib/recap/` (state.ts 17 cas pure state machine + calculations.ts 33 cas pure bilan/refloat/snapshot + 8 gated check-status), `lib/__tests__/` (auth-reducer 14 + logger 11 + contribution-calculator 8 + query-client), `components/__tests__/` (a11y-audit 19 dont 12 focus-trap `expectEscClose`), `components/ui/__tests__/` (DecimalFormInput 8 + ModalCloseX 4), RTL forms (64+ cas / 15 fichiers).
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 424 non-gated / 101 gated. 31 routes API (sprint 05 +start +status). 14 RPCs pinnées + 21 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` untracked, sprints 01-05/17 livrés (Clean-Slate-Recap 2026-05-23 + Migrations-V3 schema 2026-05-24 + State-Lock-Schemas 2026-05-24 + Calculations 2026-05-23 + Endpoints-START-STATUS-proxy 2026-05-25).
