...
- **Monthly Recap V3** : en cours d'implémentation, voir `prompt-montly-recap/` pour la spec. Sprints 01-07/17 livrés (Clean-Slate / Migrations / State+Schemas / Calculs / Endpoints START+STATUS+proxy / Endpoints flow positif / Endpoints flow négatif). Fondations TS dans [lib/recap/](lib/recap/) : `state.ts` (RecapStep + transitions), `check-status.ts` (RecapStatusKind + `.maybeSingle()` + lock), `lock.ts` (`isUserLocked`/`isRecapBlocking`), `calculations.ts` (4 fonctions pures cents-precise), `load-summary.ts` (Promise.all FinancialData + budgets + spent + piggy + bank), `active-recap.ts` (`getActiveRecap` partagé cross-sprint 06-09), `actions-positive.ts` (sprint 06 `executeTransferSurplusesToPiggy` + `executeTransformRemainingToSavings`), `actions-negative.ts` (sprint 07 3 execute helpers flow négatif + 2 purs `sumSnapshotValues`/`computeDeficitRemaining` + `RecapActionError`). RPC `start_monthly_recap` (discriminant `result` ∈ created|resumed|completed|locked_by_other). 7 endpoints sous `app/api/monthly-recap/*` (start, status, 2 positifs, 3 négatifs : refloat-piggy/refloat-savings/save-snapshot — ce dernier écrit `monthly_recaps.budget_snapshot_data` JSONB différé) via `withAuthAndProfile` + `parseBody`/`parseQuery`. **Gating proxy.ts actif** : `/dashboard` + `/group-dashboard` redirigent sur `/monthly-recap?context=X` tant que `isRecapBlocking(status)` ; cookie httpOnly 5min `recap-ok-{ctx}-{YYYY}-{MM}` cache `completed`. Schémas Zod du flow restant dans [lib/schemas/recap.ts](lib/schemas/recap.ts).
...
| Tests non-gated passants               | **436**                   | `pnpm test:run` (sprint 07 V3 : +12 actions-negative pure helpers)                                                               |
| Tests gated skipped (sans env vars)    | **140**                   | idem (sprint 07 V3 : +24 = 8 refloat-from-piggy + 8 refloat-from-savings + 8 save-budget-snapshot sous `SUPABASE_RECAP_TESTS=1`) |
| Routes API                             | **36**                    | `pnpm build` (sprint 07 V3 : +3 endpoints refloat-from-piggy / refloat-from-savings / save-budget-snapshot)                      |
...
- **SUPABASE_RECAP_TESTS=1** : 60 cas V3 — 8 `checkRecapStatus` + sprint 05 (13 cas start+status) + sprint 06 (15 cas transfer-surpluses-to-piggy + transform-remaining) + sprint 07 (24 cas refloat-from-piggy + refloat-from-savings + save-budget-snapshot, 8 chacun : happy/partial/overflow/no_deficit/completed/not_initiator/invalid_step + cents-precise ou overwrite).
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 436 non-gated / 140 gated. 36 routes API. 14 RPCs pinnées + 21 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` (rappel "relire 00-README + 00-Detailed_feature" en tête de chaque prompt 01-17), sprints 01-07/17 livrés (cf. §5 + Part 19).

**Historique** — 18 parts `.claude/history/roadmap-detailed-NN-*.md` (119 sprints) :
