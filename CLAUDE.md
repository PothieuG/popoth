...
- **Monthly Recap V3** : en cours d'implémentation, voir `prompt-montly-recap/` pour la spec. Sprints 01 Clean-Slate-Recap (2026-05-23) + 02 Migrations-V3 (2026-05-24, table `monthly_recaps` + state machine + lock + snapshot JSONB + carry-over flags) + 03 State+Schemas (2026-05-24) + 04 Calculs purs (2026-05-23, commit ec47594) livrés. Fondations TS dans [lib/recap/](lib/recap/) : `state.ts` (RecapStep + isAdvanceAllowed + nextRequiredStep), `check-status.ts` (discriminated union `RecapStatusKind` + `.maybeSingle()` + lock detection via `started_by_profile_id`), `lock.ts` (helpers purs `isUserLocked`/`isRecapBlocking`), `calculations.ts` (4 fonctions pures cents-precise : `computeBudgetSurplus`, `computeRecapSummary`, `computeProportionalSavingsRefloat`, `computeProportionalBudgetSnapshot` Option B pool=estimatedAmount), `types.ts` (interfaces partagées `BudgetSummary`/`RecapSummary`/`RefloatProportionalAllocation`). Schémas Zod des 8 endpoints futurs dans [lib/schemas/recap.ts](lib/schemas/recap.ts). Pas de gating proxy.ts pour l'instant ; sera réintroduit en sous-tâche dédiée (sprint 09+).
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 424 non-gated / 88 gated. 29 routes API (post Clean-Slate-Recap). 13 RPCs pinnées + 20 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` untracked, sprints 01-04/17 livrés (Clean-Slate-Recap 2026-05-23 + Migrations-V3 schema 2026-05-24 + State-Lock-Schemas 2026-05-24 + Calculations 2026-05-23).
