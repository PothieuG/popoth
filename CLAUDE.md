...
| Tests non-gated passants               | **626**                   | `pnpm test:run`                                                                                                          |
| Tests gated skipped (sans env vars)    | **187**                   | idem (`SUPABASE_*_TESTS=1` activent)                                                                                     |
...
**État global** : Score ~100/100. Lint 0/0. Tests 626 non-gated / 187 gated. 40 routes API. 19 RPCs + 28 fn versionnées (§5.5). **Monthly Recap V3** sprints 01-17 livrés (cf. §5 + Part 24).

**Historique** — 24 parts `.claude/history/roadmap-detailed-NN-*.md` (122 sprints) :
