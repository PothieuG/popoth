...
- Régénérer les types après changement de schéma : `pnpm db:types` (sans redirection — le script s'écrit lui-même dans `lib/database.types.ts` depuis Sprint Polish-CI / D1). Les RPC service-role-only sont incluses depuis le passage à `--linked` (Sprint Cleanup-Legacy / C1) — pas besoin de réaugmenter.
...
- ⏭️ **Sprint Hygiene-CI** ([prompt-00-executive-summary-v10.md](prompts/prompt-00-executive-summary-v10.md)) : 3 traces résiduelles surfacées par Sprint Polish-CI — E1 ajouter `.gitattributes` avec `* text=auto eol=lf` (élimine les warnings `LF will be replaced by CRLF` à chaque commit Windows + obsolète le fix D2 en steady state), E2 nouveau `pnpm db:check-types-fresh` qui détecte un `lib/database.types.ts` désynchronisé par rapport à prod (complète `db:check-drift` qui ne couvre pas les types générés), E3 follow-up observation manuelle du cron weekly D5.
