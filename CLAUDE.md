...
- **Monthly Recap V3** : sprints 01-11/17 livrés (spec `prompt-montly-recap/`). Fondations [lib/recap/](lib/recap/) (state, check-status, lock, calculations, load-summary, active-recap, actions-\*), Zod [lib/schemas/recap.ts](lib/schemas/recap.ts), 10 endpoints `app/api/monthly-recap/*` via `withAuthAndProfile` (sprint 11 ajoute `advance-step` générique avec validation `isAdvanceAllowed`). **Gating `proxy.ts`** : `/dashboard` + `/group-dashboard` redirigent sur `/monthly-recap?context=X` tant que `isRecapBlocking(status)` ; cookie httpOnly 5min `recap-ok-{ctx}-{Y}-{M}` cache `completed`. Seeds CLI [scripts/seed-recap/](scripts/seed-recap/README.md) (27 scénarios). **UI sprints 10-11** : wizard shell + frieze + lock screen + WelcomeStep + SummaryStep + BilanBlock + 2 drawers indicatifs sous `components/monthly-recap/*` ; hook [hooks/useMonthlyRecap.ts](hooks/useMonthlyRecap.ts) (useQuery + `useStartRecap`/`useAdvanceStep`) ; helper [lib/format-currency.ts](lib/format-currency.ts) (2 décimales). ⚠️ Re-entrée wizard = retour EXACT step + sub-state (cf. memory `feedback_recap_exact_reentry`).
...
| Tests non-gated passants               | **505**                   | `pnpm test:run`                                                                                                          |
| Tests gated skipped (sans env vars)    | **164**                   | idem (`SUPABASE_*_TESTS=1` activent)                                                                                     |
| Routes API                             | **38**                    | `pnpm build`                                                                                                             |
...
- **SUPABASE_RECAP_TESTS=1** : 78 cas V3 — 8 `checkRecapStatus` + sprint 05 (13 cas start+status) + sprint 06 (15 cas transfer-surpluses-to-piggy + transform-remaining) + sprint 07 (24 cas refloat-from-piggy + refloat-from-savings + save-budget-snapshot, 8 chacun : happy/partial/overflow/no_deficit/completed/not_initiator/invalid_step + cents-precise ou overwrite) + sprint 08 (18 cas update-salaries 8 + complete 10 : happy/idempotency/not_initiator/invalid_step/empty-snapshot/no-tx/group-filter/prior-carried-untouched/bogus-budget-id).
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 505 non-gated / 164 gated. 38 routes API. 16 RPCs pinnées + 23 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` (rappel "relire 00-README + 00-Detailed_feature" en tête de chaque prompt 01-17), sprints 01-11/17 livrés (cf. §5 + Part 20).

**Historique** — 20 parts `.claude/history/roadmap-detailed-NN-*.md` (121 sprints) :
