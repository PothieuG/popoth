...
- **Monthly Recap V3** : en cours d'implémentation, voir `prompt-montly-recap/` pour la spec. Sprints 01-06/17 livrés (Clean-Slate / Migrations / State+Schemas / Calculs / Endpoints START+STATUS+proxy / Endpoints flow positif). Fondations TS dans [lib/recap/](lib/recap/) : `state.ts` (RecapStep + transitions), `check-status.ts` (RecapStatusKind + `.maybeSingle()` + lock), `lock.ts` (`isUserLocked`/`isRecapBlocking`), `calculations.ts` (4 fonctions pures cents-precise), `load-summary.ts` (Promise.all FinancialData + budgets + spent + piggy + bank), `active-recap.ts` (sprint 06 `getActiveRecap` partagé cross-sprint 06-09), `actions-positive.ts` (sprint 06 `executeTransferSurplusesToPiggy` + `executeTransformRemainingToSavings`, boucles fail-soft). RPC `start_monthly_recap` (discriminant `result` ∈ created|resumed|completed|locked_by_other). 4 endpoints `app/api/monthly-recap/{start,status,transfer-surpluses-to-piggy,transform-remaining-surpluses-to-savings}/route.ts` via `withAuthAndProfile` + `parseBody`/`parseQuery`. **Gating proxy.ts actif** : `/dashboard` + `/group-dashboard` redirigent sur `/monthly-recap?context=X` tant que `isRecapBlocking(status)` ; cookie httpOnly 5min `recap-ok-{ctx}-{YYYY}-{MM}` cache `completed`. Schémas Zod du flow restant dans [lib/schemas/recap.ts](lib/schemas/recap.ts).
...
| -------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EXPECTED_RPCS`                        | **14**                    | [scripts/check-rpcs.mjs](scripts/check-rpcs.mjs) (sprint 05 V3 : +`start_monthly_recap`)                                                              |
...
| Tests non-gated passants               | **424**                   | `pnpm test:run` (sprint 04 V3 : +33 cas pure calculations)                                                                                            |
| Tests gated skipped (sans env vars)    | **116**                   | idem (sprint 06 V3 : +15 gated transfer-surpluses-to-piggy (9) + transform-remaining (6) sous `SUPABASE_RECAP_TESTS=1`)                               |
| Routes API                             | **33**                    | `pnpm build` (sprint 06 V3 : +POST /api/monthly-recap/transfer-surpluses-to-piggy, +POST /api/monthly-recap/transform-remaining-surpluses-to-savings) |
| Functions DB versionnées               | **21/21**                 | `pnpm db:audit-functions` (sprint 05 V3 : +`start_monthly_recap`)                                                                                     |
...
- **Total** : 424 non-gated + 101 gated skipped (sans env vars).
...
- **SUPABASE_RECAP_TESTS=1** : 36 cas V3 — 8 `checkRecapStatus` (4 fixtures + orphan + 2 errors) + sprint 05 routes : 7 `POST /api/monthly-recap/start` + 6 `GET /api/monthly-recap/status` + sprint 06 routes : 9 `POST /transfer-surpluses-to-piggy` (happy 3 budgets / partial / surplus=0 filter / completed→404 / no recap / not_initiator / invalid_step / 2 Zod) + 6 `POST /transform-remaining-surpluses-to-savings` (happy / no-surplus advance / step 409 / no recap / not_initiator / Zod).

### Tests non-gated par module

Couverture par dossier : `lib/finance/` (calc-rtl 19 + snapshots 5), `lib/api/` (parse-body 9, finance/expenses-add-with-logic 5 PIN ATOMIC CONTRACT), `app/api/savings/transfer/` (4 PIN ATOMIC CONTRACT), `app/api/monthly-recap/` (sprint 05 V3 : 7 gated start + 6 gated status ; sprint 06 V3 : 9 gated transfer-surpluses-to-piggy + 6 gated transform-remaining, `withAuthAndProfile` mocké), `lib/schemas/` (10 fichiers : 9 post Clean-Slate + recap V3 40 cas), `lib/recap/` (state.ts 17 cas pure state machine + calculations.ts 33 cas pure bilan/refloat/snapshot + 8 gated check-status), `lib/__tests__/` (auth-reducer 14 + logger 11 + contribution-calculator 8 + query-client), `components/__tests__/` (a11y-audit 19 dont 12 focus-trap `expectEscClose`), `components/ui/__tests__/` (DecimalFormInput 8 + ModalCloseX 4), RTL forms (64+ cas / 15 fichiers).
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 424 non-gated / 116 gated. 33 routes API. 14 RPCs pinnées + 21 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` untracked, sprints 01-06/17 livrés (cf. §5 + Part 19).

**Historique** — 18 parts `.claude/history/roadmap-detailed-NN-*.md` (118 sprints) :
