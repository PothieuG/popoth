...
- **Monthly Recap V3** : sprints 01-10/17 livrés (cf. spec `prompt-montly-recap/`). Fondations TS dans [lib/recap/](lib/recap/) : `state.ts`, `check-status.ts`, `lock.ts`, `calculations.ts`, `load-summary.ts`, `active-recap.ts`, `actions-{positive,negative,salary,finalize}.ts`. RPCs `start_monthly_recap`, `finalize_recap_apply_snapshot`, `process_recap_transactions`. 9 endpoints `app/api/monthly-recap/*` via `withAuthAndProfile`. **Gating `proxy.ts`** : `/dashboard` + `/group-dashboard` redirigent sur `/monthly-recap?context=X` tant que `isRecapBlocking(status)` ; cookie httpOnly 5min `recap-ok-{ctx}-{Y}-{M}` cache `completed`. `complete` idempotent, apply-snapshot + process-tx fail-soft. Zod dans [lib/schemas/recap.ts](lib/schemas/recap.ts). **Sprint 09** : 27 scénarios seed CLI [scripts/seed-recap/](scripts/seed-recap/README.md). **Sprint 10 (UI)** : `app/monthly-recap/page.tsx` + `components/monthly-recap/{RecapShell,RecapWizard,RecapProgressFrieze,GroupLockScreen,steps/*}` + `hooks/useMonthlyRecap.ts` (TanStack Query, staleTime 30s).
...
| Tests non-gated passants               | **459**                   | `pnpm test:run`                                                                                                          |
...
**État global** : Score audit estimé ~100/100. Lint baseline 0/0. Tests 447 non-gated / 158 gated. 37 routes API. 16 RPCs pinnées + 23 functions versionnées (cf. §5.5). **Monthly Recap V3 en cours** — spec sous `prompt-montly-recap/` (rappel "relire 00-README + 00-Detailed_feature" en tête de chaque prompt 01-17), sprints 01-08/17 livrés (cf. §5 + Part 20).
