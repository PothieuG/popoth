...
- **Monthly Recap V3** (sprints 01-15/17) : [lib/recap/](lib/recap/) + [schemas/recap.ts](lib/schemas/recap.ts), 11 endpoints via `withAuthAndProfile`. `proxy.ts` redirige `/dashboard` vers `/monthly-recap?context=X` tant que `isRecapBlocking(status)` (cookie 5min cache `completed`). Wizard 6 écrans + refloat sous `components/monthly-recap/*` ; [useMonthlyRecap.ts](hooks/useMonthlyRecap.ts). Seeds [scripts/seed-recap/](scripts/seed-recap/README.md). ⚠️ Re-entrée = EXACT step + sub-state (memory `feedback_recap_exact_reentry`). **Abandonné** (période antérieure non close) : `start_monthly_recap` rembourse les refloats → tirelire + `abandoned_at` + `recovery_data` ([Part 41](.claude/history/roadmap-detailed-41-abandoned-recap-recovery.md)).
...
- ✅ **Sprints 0 / DB / Refactor / Hardening** (C1-C3, D1-D5, R2/R4, H1/H4) : durcissement CI / RLS / atomicité / drift. **+ Security-RLS** (2026-06-03) : RLS sur `monthly_recaps` (seule table publique sans RLS — expo anon key), `ensure_rls` event-trigger restauré en prod (cause racine), guard `pnpm db:check-rls` dans `verify`. → [Part 40](.claude/history/roadmap-detailed-40-security-rls-monthly-recaps.md).
...
- **`pnpm self-update`** sans version explicite — bumpe le pin `packageManager` (incident 2026-05-20).
...
- **SUPABASE_RECAP_TESTS=1** : V3 start/status, flux positif/négatif, salary, complete, `checkRecapStatus`. Détails Part 19-21 + 41.
...
`pnpm verify` (DX-Verify / G1) — détail des checks dans le tableau §3. Skip-friendly sans env vars.
