...
- ⏭️ **Sprint DX-Verify** ([prompt-00-executive-summary-v12.md](prompts/prompt-00-executive-summary-v12.md)) : G1 ajouter `pnpm verify` meta-script consolidant la sanity sweep des 8 commandes post-sprint (réduit le risque d'oubli + mitige le `STATUS_STACK_BUFFER_OVERRUN` Windows observé en chain), G2 `.github/dependabot.yml` pour auto-PR weekly npm + GH Actions (le filet CI Sprint Code-CI / F1 + Audit-Functions-v2 / B3 gate les PRs), G3 statuer sur le défer Node 24 maintenant que la date de ré-éval (avril 2026) est passée et que la forced switch (2 juin 2026) est <1 mois. 3-4 commits, score estimé ~81/100.
- ⏭️ **GH Actions Node.js 24 migration** (Sprint Polish-CI / D6 défer, ré-évaluation prévue dans Sprint DX-Verify / G3) : `actions/checkout@v4`, `setup-node@v4`, `pnpm/action-setup@v4` warnent "Node.js 20 deprecated, forced to 24 starting June 2nd, 2026". Warning observé en CI sur Sprint Code-CI / F1 verif (2 runs). Stratégie initiale : laisser auto-upgrade arriver, observer, ajuster si breakage. **Date de ré-éval avril 2026 dépassée** — décision Sprint DX-Verify / G3.
