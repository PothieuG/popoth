...
- **Monthly Recap V3** (sprints 01-15/17) : [lib/recap/](lib/recap/) + [schemas/recap.ts](lib/schemas/recap.ts), 11 endpoints via `withAuthAndProfile`. `proxy.ts` redirige `/dashboard` vers `/monthly-recap?context=X` tant que `isRecapBlocking(status)` (cookie 5min cache `completed`). Wizard 6 écrans + refloat sous `components/monthly-recap/*` ; [useMonthlyRecap.ts](hooks/useMonthlyRecap.ts) (useQuery + 10 mutations). Seeds [scripts/seed-recap/](scripts/seed-recap/README.md). ⚠️ Re-entrée = EXACT step + sub-state (memory `feedback_recap_exact_reentry`).
...
`pnpm verify` (DX-Verify / G1) : `typecheck` + `test:run` + 7 `db:*` checks fail-fast. ~36s local. Tests gated skip-friendly sans env vars.
...
**Historique** — 40 parts (152 sprints) :
