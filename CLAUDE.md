...
- ✅ **Sprint DX-Verify** ([prompt-00-executive-summary-v12.md](prompts/prompt-00-executive-summary-v12.md)) : G1 [package.json](package.json) — script `pnpm verify` ajouté (chaîne `typecheck` + `test:run` + 6 `db:*` checks avec `&&` fail-fast, ~36s en local, exit 0 vérifié sur cleanup, mitige `STATUS_STACK_BUFFER_OVERRUN` Windows en spaçant les spawns + n'exécute pas les checks downstream sur fail). G2 [.github/dependabot.yml](.github/dependabot.yml) ajouté : auto-PR weekly lundi 08:00 Europe/Paris pour npm (avec groups `radix-ui` / `supabase` / `eslint` / `vitest` pour éviter la fragmentation) et github-actions (les futures PRs `actions/*@v5` fermeront le défer Node 24 en follow-up). PRs targettent `cleanup`, gates F1+B3 actifs. **G3 deferred to post-G2 observation** : pas de bump manuel ce sprint — Dependabot proposera `actions/*@v5` après G2, on les mergera en follow-up. **Triage 1re vague Dependabot** également follow-up post-sprint (1-2h estimé, e.g. ne PAS merger `eslint-config-next` 15→16 réservé Sprint 1). Pas de migration DB. Livré 2026-05-07, 3 commits sur `cleanup` (G1 / G2 / closeout), score estimé ~81/100.
- ⏭️ **GH Actions Node.js 24 migration** (Sprint Polish-CI / D6 défer, statué Sprint DX-Verify / G3) : déféré au follow-up post-G2 — Dependabot ouvrira les PRs `actions/checkout@v5` / `actions/setup-node@v5` / `pnpm/action-setup@v5` lors de la première vague. Forced switch 2026-06-02 (~3-4 semaines). Si Dependabot ne propose pas v5 à temps, fallback : bump manuel direct dans les 3 YAML.
