...
| `pnpm db:check-rpcs`                       | Vérifie présence des **25 RPCs pinnées** dans `pg_proc`                                                                                   |
...
- **Projets d'épargne** (sprints PÉ 01-11, [Part 29-31](.claude/history/roadmap-detailed-29-projets-epargne.md)) : `savings_projects` + 4 RPCs. `monthly_allocation` = budget virtuel dans `totalEstimatedBudgets`. Cascade refloat entre savings et budget snapshot dans `BilanNegativeStep` (sprint PÉ 09) ; finalize `apply_recap_projects_snapshot` (sprint PÉ 10) crédite `amount_saved += monthly-refund` + shift deadline frac. Seed `project-deficit-refloat.mjs`. ⚠️ Sanctuarisés (cascade auto-piggy ne touche pas).
...
- **Globals** : **0** `declare global` dans le code.
- **Distinction calculs finance** : [contribution-calculator.ts](lib/contribution-calculator.ts) (budget-allocation, salary-split, pur, consumer `ProfileSettingsCard`) ≠ [finance/income-compensation.ts](lib/finance/income-compensation.ts) (async, alimente RAV via `_loadFinancialData`). Noms voisins, orthogonaux.
...
| Tests non-gated passants               | **758**                   | `pnpm test:run`                                                                                                          |
| Tests gated skipped                    | **223**                   | idem (`SUPABASE_*_TESTS=1` activent)                                                                                     |
...
**État global** : Score ~100. Lint 0/0. Tests 758/223. 44 routes. 25 RPCs + 34 fn. **MRv3** 01-17 + **PÉ** 01-11 livrés (seed PÉ + push prod sprint 11).

**Historique** — 31 parts (137 sprints) :
