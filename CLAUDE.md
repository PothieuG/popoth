...
Prod hébergée sur Supabase (`jzmppreybwabaeycvasz`). Audit complet 2026-04 dans [docs/audit/](docs/audit/) (score 47/100 avant Sprint 0, ~51 après Sprint 0, ~58 après Sprint DB, ~62-65 après Sprint Refactor, ~70 après Sprint Hardening, ~73 après Sprint Polish, ~75 après Sprint Audit-Triggers, ~76 après Sprint Audit-Functions-v2, ~77 après Sprint Cleanup-Legacy). Carte du schéma à jour dans [docs/db/SCHEMA.md](docs/db/SCHEMA.md) (inclut l'inventaire complet des triggers prod).
...
| `pnpm db:types` | Régénère `lib/database.types.ts` depuis le schéma prod (Sprint DB / D6). **⚠️ Bug à fixer (Sprint Polish-CI / D1)** : la redirection `pnpm db:types > lib/database.types.ts` capture l'en-tête wrapper pnpm dans le fichier. Workaround actuel : `pnpm supabase gen types typescript --linked --schema public > lib/database.types.ts`. |
| `pnpm db:check-drift` | Compare prod ↔ baseline `20260101000000_remote_schema.sql`. Exit 0 = clean, 1 = drift (Sprint Refactor / R4). **⚠️ Faux positif Windows** : sur un working copy CRLF (post-`git checkout`), le détecteur signale chaque ligne comme différente. Bug à fixer (Sprint Polish-CI / D2). Workaround : re-export via `node scripts/export-schema.mjs supabase/migrations/20260101000000_remote_schema.sql` qui réécrit en LF. |
...
- ⏭️ **Sprint Polish-CI** ([prompt-00-executive-summary-v9.md](prompts/prompt-00-executive-summary-v9.md)) : 4 fixes surfacés par Cleanup-Legacy à traiter — D1 fix pollution wrapper `pnpm db:types` (le redirect `> lib/database.types.ts` capture l'en-tête pnpm), D2 normaliser fins de ligne dans `check-drift.mjs` (faux positif Windows post-`git checkout`), D3 supprimer l'augmentation `lib/database.ts` redevenue redondante (les RPC C3 sont maintenant dans les types générés), D4 ajouter les YAML eux-mêmes au path filter du PR-time gate (B3 cassé sans être détecté par lui-même), D5 valider manuellement le cron weekly via `workflow_dispatch`. D6 GH Actions Node.js 24 défer.
