...
| Tests non-gated passants               | **631**                   | `pnpm test:run`                                                                                                          |
| Tests gated skipped (sans env vars)    | **198**                   | idem (`SUPABASE_*_TESTS=1` activent)                                                                                     |
...
**État global** : Score ~100/100. Lint 0/0. Tests 631 non-gated / 198 gated. 41 routes API. 19 RPCs + 28 fn versionnées (§5.5). **Monthly Recap V3** sprints 01-17 livrés (cf. §5 + Part 24). Salary-Edit-Gating → Part 25.

**Historique** — 25 parts `.claude/history/roadmap-detailed-NN-*.md` (123 sprints) :
