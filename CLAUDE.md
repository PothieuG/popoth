...
| `pnpm db:check-types-fresh` | Vérifie que `lib/database.types.ts` correspond à ce que `supabase gen types --linked` produirait à l'instant T. Exit 0 = synchro, 1 = stale + diff sur stdout, 2 = fatal (Sprint Hygiene-CI / E2). Comble le trou que `db:check-drift` ne couvre pas (drift schéma SQL ≠ drift types générés). À lancer après chaque migration touchant une table/colonne/RPC. |
...
- E3 — **À OBSERVER MANUELLEMENT** (re-run du défer Sprint Polish-CI / D5, étendu) : aller sur https://github.com/PothieuG/popoth/actions/workflows/db-drift-check.yml, **Run workflow** sur `cleanup`, vérifier les **5** steps verts (pnpm/action-setup, db:check-drift, db:check-rpcs, db:check-functions, **db:check-types-fresh**) et que "Open issue on failure" ne se déclenche pas. C3 a fixé le YAML et E2 a élargi la batterie de checks ; il reste à confirmer qu'un run cron exit 0 en mode `schedule` (vs `pull_request`).
...
- ✅ **Sprint Hygiene-CI** ([prompt-00-executive-summary-v10.md](prompts/prompt-00-executive-summary-v10.md)) : E1 `.gitattributes` ajouté avec `* text=auto eol=lf` (élimine le warning `LF will be replaced by CRLF` sur chaque commit Windows ; **renormalize a été un no-op** car repo + working tree étaient déjà LF en storage — il manquait juste la déclaration explicite pour que tous les clones convergent côté policy ; le fix D2 reste comme défense en profondeur). E2 `pnpm db:check-types-fresh` ([scripts/check-types-fresh.mjs](scripts/check-types-fresh.mjs)) : détecte un `lib/database.types.ts` désynchronisé par rapport à prod via `supabase gen types --linked` + line-by-line diff. Wirage : step ajouté à db-drift-pr.yml ET db-drift-check.yml (`if: always()`) ; path filter du PR-time gate étendu à `lib/database.types.ts`. **Verif end-to-end** : exit 0 clean, 1 sur édition manuelle avec diff résumé, 2 sur token absent. E3 cron weekly à observer manuellement (re-run élargi du défer D5 — cette fois 5 steps verts à confirmer). Livré 2026-05-07, 2 commits code + closeout sur `cleanup`, score estimé ~79/100
