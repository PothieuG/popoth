...
| `pnpm db:types` | Régénère `lib/database.types.ts` depuis le schéma prod (Sprint DB / D6). Le script écrit lui-même dans le fichier — appeler **sans redirection** (`pnpm db:types`, pas `pnpm db:types > ...`). Sprint Polish-CI / D1 a déplacé le `>` à l'intérieur du script pour ne plus capturer l'en-tête wrapper pnpm. |
...
- **Pour DROP une fonction (ou tout objet DB) déjà capturée et confirmée dead code** (cas Sprint Cleanup-Legacy / C1) : workflow miroir du capture-then-drop. Pré-requis : la migration de capture existe déjà, vérifier 0 callsite app (Grep), 0 référence GitHub Actions, 0 cron Supabase / webhook / Edge Function (dashboard). Pattern :
  ...
  5. Régénérer types : `pnpm supabase gen types typescript --linked --schema public > lib/database.types.ts` (NE PAS utiliser `pnpm db:types` qui ajoute des en-têtes pnpm dans le fichier — bug à fixer un jour). Verifier `pnpm typecheck` exit 0.
...
- ⏭️ **Sprint Code-CI** ([prompt-00-executive-summary-v11.md](prompts/prompt-00-executive-summary-v11.md)) : 2 traces résiduelles surfacées par Sprint Hygiene-CI — F1 ajouter `.github/workflows/code-checks.yml` (PR-time gate `pnpm typecheck` + `pnpm test:run`, premier filet code-side après 6 sprints DB-side), F2 aligner `pnpm db:types` sur `--project-id` (cohérence avec `db:check-types-fresh` post E2 hotfix, élimine la dépendance à `supabase link` pour les fresh clones). F3 observation passive du cron lundi suivant pour confirmer mode `schedule` stable. Pas de migration DB. Score estimé post-sprint : ~80/100 (premier vrai franchissement du seuil 80).
