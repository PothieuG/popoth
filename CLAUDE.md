...
| Tests non-gated passants               | **791**                   | `pnpm test:run`                                                                                                          |
...
**État global** : Score ~100. Lint 0/0. Tests 791/227. 44 routes. 25 RPCs + 36 fn. **MRv3** 01-17 + **PÉ** 01-12 livrés. ⚠️ **État ouvert** : la dette `carryover_spent_amount` peut piéger un budget quand `carryover ≥ estimated` — fix immédiat livré (filtre date + dropdown align) mais finalité UX/sémantique non atteinte, cf. [Part 34](.claude/history/roadmap-detailed-34-allow-negative-rav-and-deficit-bug.md).

**Historique** — 34 parts (142 sprints) :
