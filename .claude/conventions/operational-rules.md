...
- ❌ **NE PAS** retourner une ligne `real_expenses` / `real_income_entries` au client sans le JOIN `created_by:profiles!<table>_created_by_profile_id_fkey(id, first_name, last_name)` (hint FK obligatoire : 2 FK vers `profiles`). **Jamais `avatar_url` dans ce JOIN** (Sprint Fix-Avatar-Payload 2026-09-11) : une photo brute de 3,7 Mo en base64 y était répétée à chaque ligne → `GET /expenses/real?group=true` en 500 après 14,7 s (cause réelle du « groupe lent », HAR prod). L'avatar du créateur est résolu côté client depuis `useGroupMembers` (prop `creatorAvatarUrl` de `TransactionListItem`). 8 sites (3 expenses-real, 3 income-real, 2 add-with-logic), pinnés par `list-routes-query-plan.test.ts`.
...
- ❌ **NE PAS** réintroduire `bilan = ravEffectif - ravEstime` (ni l'additive `+ ravEstime`) dans `computeRecapSummary` ([lib/recap/calculations.ts](../../lib/recap/calculations.ts)). Formule canonique : **`bilan = ravEffectif`** (le reste à vivre effectif tel quel) depuis Sprint Bilan-Equals-RavEffectif (2026-06-01, remplace Fix-Recap-Bilan-Formula). Sémantique : `> 0` ⇒ argent « en plus » hors budget, **balayé vers la tirelire** au « Continuer » positif (`executeTransformRemainingToSavings` → `updatePiggyBank` du montant, disjoint des sous-dépenses budget — pas de double-comptage car la tirelire n'entre pas dans le RAV) ; `< 0` ⇒ déficit à renflouer `= |ravEffectif|` (flux négatif, `computeDeficitRemaining` lit `summary.bilan`) ; `= 0` ⇒ équilibre. `ravEstime` n'est plus qu'une métrique affichée. Leçon conservée : pour formule métier critique, ≥ 1 cas test EN VALEURS DE LA SPEC. Cascade : `calculations.ts` (+docstring) + `types.ts` + `load-summary.ts` + tests `calculations`/`load-summary` + UI `BilanBlock`/`BilanPositiveStep`.
...
- ❌ **NE PAS** réintroduire le pattern read-persisted-then-override sur `/api/finance/summary` (ou toute route retournant `FinancialData`) : `persistedRav = await getRavFromDatabase(...)` puis `await getProfileFinancialData(...)` puis `financialData.remainingToLive = persistedRav`. Off-by-one cache : `getProfileFinancialData` recompute + persiste via `saveRavToDatabase` side-effect, mais l'API retournait la valeur lue AVANT. Symptôme : "j'ajoute une dépense, RAV ne bouge pas sans refresh". Fix Sprint Fix-Summary-RAV-Stale-Cache 2026-05-21 : drop `getRavFromDatabase` import + override + branche `if (shouldRecalculate)`. Retourner le résultat de `getProfileFinancialData`/`getGroupFinancialData` directement — exception : `/api/finance/rav` (endpoint dédié).
- ❌ **NE PAS** calculer `budgetRemaining` sans `carryover_spent_amount` ni filtrer `applied_to_balance_at` dans le récap — miroir RAV/dashboard. Cf. [Part 34](../history/roadmap-detailed-34-allow-negative-rav-and-deficit-bug.md) sprints Fix-Preview-Allocation-Carryover + Fix-Recap-Surplus-Inconsistency.
