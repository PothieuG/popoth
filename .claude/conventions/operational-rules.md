...
- ❌ **NE PAS** réintroduire une page `app/dev/recap/` ni des routes `app/api/debug/recap/{scenarios,seed,reset}/route.ts` ni un module `lib/dev/{recap-scenarios,apply-scenario}.ts` — le prompt sprint 09 original les prévoyait, **le pivot validé avec le user 2026-05-23** les a remplacés par 1 script CLI Node = 1 scénario sous [scripts/seed-recap/](../../scripts/seed-recap/README.md). Raison : tester l'UX réelle de bout en bout dans le navigateur avec **vraies data dev** (users A=`0679b0f9…`/B=`bb53b671…`/groupe `92dbf6f2…`) plutôt que via une UI de seed in-app qui dupliquait la logique d'orchestration. Le bypass des conditions de lancement passe par INSERT direct dans `monthly_recaps` (cf. helper `seedRecapRow` du `_lib.mjs`) — pas de nouveau endpoint. Si une demande future veut une route admin/dev runtime, vérifier d'abord si un script CLI ne fait pas déjà le job.
