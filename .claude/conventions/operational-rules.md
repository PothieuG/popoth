...
`EXPECTED_RPCS = 25` pinnés ([scripts/check-rpcs.mjs](../../scripts/check-rpcs.mjs)). Hors-table : `toggle_real_expense_applied_to_balance` + `toggle_real_income_applied_to_balance` (Long-Press-Toggle), `start_monthly_recap` (sprint 05 V3), `finalize_recap_apply_snapshot` + `process_recap_transactions` (sprint 08 V3), `apply_recap_projects_snapshot` (sprint 01, wiring sprint 10). Sprints 02-03 ajoutent `projects.ts` + `projects-meta.ts` (`_loadFinancialData` 3.bis), cf. [Part 29](../history/roadmap-detailed-29-projets-epargne.md).
