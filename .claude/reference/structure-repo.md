...
app/                       # App Router (pages + API routes)
  ...
  api/
    ...
    monthly-recap/         # ✅ Sprint 05 V3 (2026-05-25) — 2 routes start (POST) + status (GET)
      start/route.ts       # POST claim lock atomique via RPC `start_monthly_recap`. Body Zod startRecapBodySchema { context }. Dispatch sur result: 200 created/resumed + summary | 409 locked_by_other | 410 already_completed | 400 Pas de groupe.
      status/route.ts      # GET ?context=profile|group. Wrap checkRecapStatus + loadRecapSummary si in_progress. 200 { status, summary | null } | 400 NO_GROUP | 404 PROFILE_NOT_FOUND.
      start/__tests__/route.integration.test.ts  # gated SUPABASE_RECAP_TESTS=1, 7 cas (created/resumed/orphan/group-created/locked_by_other/completed/no-group)
      status/__tests__/route.integration.test.ts # gated, 6 cas (no_recap/in_progress+summary/completed/locked_by_other/no-group/missing-query)
...
lib/
  ...
  recap/                   # ✅ Sprint State-Lock-Schemas-V3 (sub-task 03/17 Monthly Recap V3, 2026-05-24) + Calculations-V3 (sub-task 04/17, 2026-05-23) + Endpoints-START-STATUS-proxy (sub-task 05/17, 2026-05-25)
    ...
    types.ts               # ✅ Sprint Calculations-V3 (2026-05-23) — 3 interfaces partagées : `BudgetSummary` (budgetId/Name + estimatedAmount + spentThisMonth + cumulatedSavings + surplus/deficit), `RecapSummary` (currentBalance + ravEstime/Effectif + totalSurplus/Savings + piggyAmount + budgets readonly + bilan + bilanSign), `RefloatProportionalAllocation` (perBudget readonly + totalAllocated + shortfall). Consommé par calculations.ts (sprint 04) et futurs actions-negative.ts (sprint 07).
    ...
    __tests__/calculations.test.ts # ✅ Sprint Calculations-V3 (2026-05-23) — 33 cas non-gated env=node : 6 computeBudgetSurplus (max-zero clamp + cents-precise 100.33−100.32=0.01 + edge zeros) + 8 computeRecapSummary (bilanSign positive/zero/negative + tri budgetId + cumulatedSavings sum cents + piggy passthrough + bilan 100.005+100.004=200.01) + 12 computeProportionalSavingsRefloat (proportional 200+100 → 66.67+33.33 / pool exact / pool<target shortfall / pool=0 / single / target=0 / cents-precise 3x100 last absorbs 0.01 / sort budgetId / sum garanti / target négatif clamp / pool=target equal split) + 7 computeProportionalBudgetSnapshot (Option B regression-guards : 30€/3×100 = 10/10/10 + non-equal 100/50/25 = 17.14/8.57/4.29 + cents target=10 → 3.33/3.33/3.34 + insufficient + pool=0 + carryover-ignored sanity + sort budgetId).
