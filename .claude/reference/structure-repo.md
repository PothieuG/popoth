...
app/                       # App Router (pages + API routes)
  ...
  api/
    ...
    monthly-recap/         # Monthly Recap V3 — 9 routes (sprint 05 start+status, sprint 06 positif×2, sprint 07 négatif×3, sprint 08 salary+complete). Toutes via `withAuthAndProfile` + `parseBody`/`parseQuery` + canonical validation chain (parseBody → context guard → getActiveRecap 404 → not_initiator 403 → invalid_step 409 with currentStep echoed) + delegate à un `executeXxx` helper de `lib/recap/actions-*.ts` (sauf start/status qui restent inline).
...
components/                # UI (shadcn/ui sous components/ui/)
  ...
  monthly-recap/{RecapShell,RecapWizard,RecapProgressFrieze,GroupLockScreen}.tsx + steps/*Step.tsx  # ✅ Sprint 10 V3 — wizard plein-écran. Shell fixed inset-0 gradient bleu-indigo. Wizard routeur kind/step via useMonthlyRecap. Frieze barre + label `role=progressbar`. LockScreen destructure logoutAndRedirect. 6 steps placeholders sprints 11-14. 12 tests RTL.
...
hooks/                     # 19 hooks React (post Clean-Slate-Recap + sprint 10 useMonthlyRecap) — ✅ Sprint 1.5 : hooks fetcher migrés sur TanStack Query (useQuery + useMutation)
  useMonthlyRecap.ts       # ✅ Sprint 10 V3 — useQuery queryKey ['monthly-recap','status',context] vers /api/monthly-recap/status. staleTime 30s. Retourne { status: RecapStatusKind, summary: RecapSummary|null }. Consommé par RecapWizard.
