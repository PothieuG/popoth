...
components/                # UI (shadcn/ui sous components/ui/)
  ...
  settings/GroupManagementPanel.tsx # ✅ Sprint Refactor-Settings-Drawer — extrait verbatim du contenu actif de l'ex-`app/settings/page.tsx:172-454` (Card Mon groupe + Card Rejoindre + ConfirmationDialog + GroupMembersWithContributionsModal). Loading overlay fullscreen (`fixed inset-0 bg-black/50`) remplacé par **snackbar non-bloquante z-[60]** (pattern mirror ProfileSettingsCard) + spinner inline dans les boutons. Skeleton `animate-pulse` léger pour Card "Mon groupe" pendant le fetch initial (au lieu du loading screen full-page). Header avec back arrow (← onBack vers main) + close X (onClose drawer entier).
